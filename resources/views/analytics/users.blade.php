            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              @forelse($users as $u)
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{{ $u->name }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{{ $u->email }}</td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    @if($roleName === 'student')
                      <span
                        class="px-2 py-1 text-xs font-semibold rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">Student</span>
                    @elseif($roleName === 'marketing_coordinator')
                      <span
                        class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">Coordinator</span>
                    @else
                      <span
                        class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">{{ ucfirst(str_replace('_', ' ', $roleName)) ?: '—' }}</span>
                    @endif
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    @if($roleName === 'student')
            ...
            <div class="p-4">
              <div class="flex items-start justify-between gap-3 mb-1">
                ...
                <div class="shrink-0">
                  @if($roleName === 'student')
                    <span
                      class="px-2 py-0.5 text-xs font-semibold rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">Student</span>
                  @elseif($roleName === 'marketing_coordinator')
                    <span
                      class="px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">Coordinator</span>
                  @else
                    <span
                      class="px-2 py-0.5 text-xs font-semibold rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">{{ ucfirst(str_replace('_', ' ', $roleName)) ?: '—' }}</span>
                  @endif
                </div>
              </div>
              <div class="flex items-center gap-3 mt-2">
                ...
                @elseif($roleName === 'marketing_coordinator')
                  <span
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round"
                        d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                ...
                <span class="text-xs text-gray-400 dark:text-gray-500">
                  Last login: {{ $u->last_login_at ? $u->last_login_at->diffForHumans() : 'Never' }}
                </span>
              </div>
            </div>
          @empty
            <div class="p-8 text-center text-gray-500 dark:text-gray-400">No users found.</div>
          @endforelse
        </div>
      </div>
            ...
            @for($i = max(1, $currentPage - 2); $i <= min($lastPage, $currentPage + 2); $i++)
              <a href="{{ request()->fullUrlWithQuery(['page' => $i]) }}"
                class="px-3 py-1.5 text-sm rounded-lg border transition-colors
                      {{ $i === $currentPage ? 'bg-[#dc2d3d] border-[#dc2d3d] text-white font-semibold' : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                {{ $i }}
              </a>
            @endfor
