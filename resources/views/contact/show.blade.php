    ...
    <main class="p-4 lg:p-8">
      <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 lg:p-8 mb-6">
          ...
          <!-- Header: avatar+info stacked on mobile, row on sm+ -->
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
            <div class="flex items-center space-x-4">
              <div
                class="w-12 h-12 lg:w-16 lg:h-16 shrink-0 bg-[#dc2d3d] rounded-full flex items-center justify-center text-white text-xl lg:text-2xl font-bold">
                {{ strtoupper(substr($contact->name, 0, 1)) }}
              </div>
              <div>
                <h2 class="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">{{ $contact->name }}</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400 break-all">{{ $contact->email }}</p>
                @if($contact->user_id)
                  <span
            ...
            <!-- Status badge: left-aligned on mobile, right on sm+ -->
          ...
          <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 mb-6">
            <div class="flex items-start space-x-3 mb-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-green-800 dark:text-green-300 whitespace-pre-wrap break-words">
                  {{ $contact->admin_response }}</p>
              </div>
            </div>
          </div>
        @endif
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          <a href="{{ route('contact.edit', $contact) }}"
            class="inline-flex items-center justify-center px-6 py-3 bg-[#dc2d3d] text-white rounded-lg font-semibold hover:bg-[#b82532] transition-colors">
          ...
          <form action="{{ route('contact.destroy', $contact) }}" method="POST"
            ...
            @method('DELETE')
            <button type="submit"
              class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors">
              <svg class="w-5 h-5 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <a href="{{ route('contact.index') }}"
            class="inline-flex items-center justify-center px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg font-semibold hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
            Cancel
          </a>
        </div>
      </div>
