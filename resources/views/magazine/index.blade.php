    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
          {{ request('trash') ? 'Deleted Magazines' : 'Magazine' }}
        </h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">
          {{ request('trash') ? 'Restore or permanently delete these entries.' : 'Published editions and articles from Kingsford University' }}
        </p>
      </div>
    ...
    @if(request('trash') && auth()->check() && auth()->user()->hasAnyRole(['marketing_manager', 'admin']))
      ...
      @if($magazines->isEmpty())
        <div class="text-center py-20 text-gray-400 dark:text-gray-600">
          ...
          <p class="text-lg font-medium">No deleted magazines</p>
        </div>
      @else
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
              @foreach($magazines as $magazine)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                  ...
                  <td class="px-6 py-4 text-gray-500 dark:text-gray-400">{{ $magazine->deleted_at->format('d M Y, H:i') }}</td>
                  <td class="px-6 py-4">
                    <div class="flex items-center justify-end gap-2">
                      ...
                      <form action="{{ route('magazine.forceDelete', $magazine->id) }}" method="POST"
                        onsubmit="return confirm('Permanently delete this magazine? Files will be removed and this cannot be undone.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
              ...
              <div class="p-5 flex flex-col flex-1 border-t border-gray-100 dark:border-gray-700">
                <div class="flex items-center justify-between mb-2">
                  <span class="inline-block px-3 py-1 text-xs font-semibold text-[#dc2d3d] bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-full">
