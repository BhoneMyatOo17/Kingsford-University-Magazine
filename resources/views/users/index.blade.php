      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 lg:p-6">
          <div class="flex items-center justify-between">
            ...
            <div
              class="w-10 h-10 lg:w-12 lg:h-12 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex items-center justify-center">
              <svg class="w-5 h-5 lg:w-6 lg:h-6 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @if(auth()->user()->hasRole('admin'))
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 lg:p-6">
            <div class="flex items-center justify-between">
              ...
              <div
                class="w-10 h-10 lg:w-12 lg:h-12 bg-orange-100 dark:bg-orange-900/30 rounded-lg flex items-center justify-center">
              ...
              @forelse($users as $user)
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      ...
                      <div class="ml-4">
                        ...
                        @elseif($user->staff)
                          <div class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ $user->staff->staff_id }}</div>
                        @endif
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    @if($user->is_active)
                      <span
                        class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">Active</span>
                    @else
                      <span
                  ...
                  @if(auth()->user()->hasRole('admin'))
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <a href="{{ route('users.show', $user) }}"
                        class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 font-medium">View</a>
                    </td>
                  @endif
                </tr>
              @empty
            ...
            <div class="p-4">
              ...
              <div class="flex items-center justify-between gap-3 mb-2">
                <div class="flex items-center gap-3 min-w-0">
                  @if($user->profile_picture)
                    <img src="{{ app(\App\Services\StorageService::class)->profilePictureUrl($user->profile_picture) }}"
                      alt="{{ $user->name }}" class="w-9 h-9 rounded-full object-cover shrink-0">
                  @else
                    <div
                      class="w-9 h-9 bg-[#dc2d3d] rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">
                      {{ strtoupper(substr($user->name, 0, 1)) }}
                    </div>
                  @endif
                  <div class="min-w-0">
                ...
                @if(auth()->user()->hasRole('admin'))
                  <a href="{{ route('users.show', $user) }}"
                    class="text-sm text-blue-600 hover:text-blue-900 dark:text-blue-400 shrink-0 font-medium">View</a>
                @endif
              </div>
              ...
              <div class="flex items-center gap-2 mb-1">
                ...
                @else
                  <span
                    class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">Inactive</span>
                @endif
              </div>
